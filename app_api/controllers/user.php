...
class User extends CI_Controller {
	...
	public function last_cleaning() {
		...
		if($access_token && $userdata) {
			...
			$onboard_date   = strtotime($data->onboard_date);
			$output['time'] = date('H:i', $onboard_date);
			$output['date'] = date('d M', $onboard_date);
			$output['year'] = date('Y', $onboard_date);
			if ($data->finish_date) {
				$output['status'] = "Done";
			} else {
			...
			if ($data) {
				...
				if ($washing_photos_data) {
					foreach ($washing_photos_data as $value) {
						if ($value->is_after) {
							$output['photos_after'][$i_after] =  "data:image/jpeg;base64,".$value->photo_url;
							$i_after++;
						} else {
							$output['photos_before'][$i_before] = "data:image/jpeg;base64,".$value->photo_url;
							$i_before++;
						}
					}
				}
